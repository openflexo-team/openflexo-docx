...
@ImplementationClass(AbstractSelectDocXParagraph.AbstractSelectDocXParagraphImpl.class)
public interface AbstractSelectDocXParagraph<AT> extends AbstractFetchRequest<DocXModelSlot, DocXDocument, DocXParagraph, AT> {
	...
	public static abstract class AbstractSelectDocXParagraphImpl<AT>
			extends AbstractFetchRequestImpl<DocXModelSlot, DocXDocument, DocXParagraph, AT> implements AbstractSelectDocXParagraph<AT> {

		private static final Logger logger = Logger.getLogger(AbstractSelectDocXParagraph.class.getPackage().getName());

		// private DataBinding<ExcelSheet> excelSheet;

		public AbstractSelectDocXParagraphImpl() {
			super();
		}
		...
		@Override
		public List<DocXParagraph> performExecute(RunTimeEvaluationContext evaluationContext) {

			DocXDocument docxDocument = getReceiver(evaluationContext);
			...
			List<DocXParagraph> selectedParagraphs = new ArrayList<>();
			selectedParagraphs.addAll(docxDocument.getElements(DocXParagraph.class));
			...
			try {
				...
				if (excelSheet != null) {
					...
					for (ExcelSheet excelSheetItem : docxDocument.getExcelSheets()) {
			...
			List<DocXParagraph> returned = filterWithConditions(selectedParagraphs, evaluationContext);
